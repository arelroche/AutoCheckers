...
class CPiece(object):
        ...
        for dc in (-1, 1) :
            if is_free(row + dr, col + dc) :
                if not capture :
                    ...
                    move.append(CAction('MOVE', (row, col), (row + dr, col + dc), promote=prom))
                ...
                if obstruction.color != self.color :
                    ...
                    move.append(CAction('CAPTURE', (row, col), (row + 2 * dr, col + 2 * dc), obstruction, prom))
                    capture = True
        if capture :
            move_new = []
            for m in move :
    ...
    def _possible_action_king(self):
        ...
        for dir in direction :
            ...
            while is_free(trow, tcol) :
                if not capture :
                    move.append(CAction('MOVE', (row, col), (trow, tcol)))
                trow += dir[0]
                tcol += dir[1]
            if board.is_free(trow + dir[0], tcol + dir[1]) :
                obstruction = board.get_piece(trow, tcol)
                if obstruction.color != self.color :
                    move.append(CAction('CAPTURE', (row, col), (trow + dir[0], tcol + dir[1]), obstruction))
                    capture = True
        if capture :
            move_new = []
            for m in move :
    ...
    def __str__(self):
            return "{ <%d , %d> <%s> <%s> }" % (self.position[0], \
                                                         self.position[1], self.is_king, \
                                                         self.color)    
